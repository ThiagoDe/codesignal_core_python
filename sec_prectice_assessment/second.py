...
# 2. If this chosen prefix contains at least two characters, cut this prefix from s and go back 
...
# Note: you can click on the prefixes and palindrome words to see the definition of the terms if 
...
# * For s = "codesignal", the output should be solution(s) = "codesignal".The initial string s = "codesignal" contains the only prefix, which is also palindrome - "c". This prefix is the longest, but doesn't contain two characters, so the algorithm ends with string "codesignal" as a result.
